...
void ProgressBar_2::updateValue(double delta)
{
    ...
    if (m_textureProgressBar != nullptr)
    {
        ...
        if (std::abs(m_parametersAmimation.targetValue - m_parametersAmimation.currentValue) > tolerance_0)
        {
            ...
            constexpr double tolerance_1 = 0.3;
...
void ProgressBar_2::updateNumber()
{
    // УРОВЕНЬ ЗДОРОВЬЯ
...
void ProgressBar_2::set_root(godot::Window* root)
{
    m_root = root;
}

void ProgressBar_2::update(double delta)
{
    godot::UtilityFunctions::print("\"ProgressBar_2::update\"");
